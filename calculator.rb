# Add your variables here
first_number = 1
def calculator(first_number= 1, second_number)
  sum = first_number.to_i + second_number.to_i
end

def calculator2(first_number = 1, second_number)

  difference = first_number.to_i - second_number.to_i

end
def calculator3(first_number = 1, second_number)
 
  product = first_number.to_i * second_number.to_i
 
end
def calculator4(first_number = 1, second_number)

  quotient = first_number.to_i / second_number.to_i
end